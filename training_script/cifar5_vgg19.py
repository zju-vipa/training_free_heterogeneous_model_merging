...
if __name__ == "__main__":
    ...
    model_dir = os.path.join(model_dir, f'vgg19x{model_width}', 'pairsplits')
    print(model_dir)
    ...
    for _ in range(split_runs):
        ...
        for j in range(models_per_run):
            for i in range(len(splits)):
                model = vgg19(w=model_width, num_classes=out_dim).cuda().train()
                ...
                save_path = os.path.join(save_dir, f'vgg19x{model_width}_v{len(os.listdir(save_dir))}.pth.tar')
