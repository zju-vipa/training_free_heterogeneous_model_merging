...
def match_tensors_zipit(
    ...
    **kwargs
    ...
    if "covariance" in metric:
        ...
        sims = metric["cossim"]
    O = sims.shape[0]
    remainder = max(model_dims) # the dim remained according to model with max dim
    ...
    merge_value = []
    prefix_model_dims = model_dims[:]
    for i in range(1,len(prefix_model_dims)):
        prefix_model_dims[i] += prefix_model_dims[i-1]
    prefix_model_dims = [0] + prefix_model_dims
...
def match_tensors_permute(metric, model_dims, get_merge_value=False, add_bias=False, **kwargs):
    """
