...
from graphs.resnet_graph import ResNetGraph
...
def run_node_experiment(node_config, experiment_config,experiment_config_b, pairs, csv_file):
    ...
    for i, pair in enumerate(tqdm(pairs, desc='Evaluating Pairs...')):
        ...
        train_loader = train_cfg["reset"] if "reset" in train_cfg  else train_cfg['full']
        reset_random()
        ...
        with torch.cuda.device(device):
            macs, params = get_model_complexity_info(base_models_a[0],(3,224,224),print_per_layer_stat=False)
            print('{}  {:<8}'.format('Model A Computational complexity: ', macs))
            macs, params = get_model_complexity_info(base_models_b[0],(3,224,224),print_per_layer_stat=False)
        ...
        def get_num_blocks(cfg_name):
            if "resnet50" in cfg_name:
                return [6,6,6,6] if "imnet" in cfg_name else [8,8,8]
            elif "resnet34" in cfg_name and "imnet" in cfg_name:
                return [4,4,4,4]
            elif "resnet44" in cfg_name:
                return [7,7,7]
            elif "resnet38" in cfg_name:
                return [6,6,6]
            elif "resnet32" in cfg_name:
                return [5,5,5]
            elif "resnet26" in cfg_name:
                return [4,4,4]
            elif "resnet20" in cfg_name:
                return [3,3,3]
            else:
                raise NotImplementedError
        ...
        for models_name,base_models,pair_idxs in zip([[config['model']['name'], config_b['model']['name']],
            ...
            graphs = [ResNetGraph(b,num_layers=len(get_num_blocks(config_name))).graphify() for b in base_models]
            # print("Node Number", [len(g.G) for g in graphs])
            ...
            with torch.cuda.device(device):
                macs, params = get_model_complexity_info(merge_model,(3,224,224),print_per_layer_stat=False)
