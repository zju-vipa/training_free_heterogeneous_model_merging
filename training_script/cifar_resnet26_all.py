...
from models.resnets import resnet26
import torch
import torchvision
...
if __name__ == "__main__":
    ...
    use_clip = False
    model_dir = f'./checkpoints_all/cifar50_{"clip" if use_clip else "logits"}_e{args.epoch}'
    
    split_runs = 1                          # num times to sample disjoint split sets
    ...
    epochs = args.epoch                            # train epochs
    
    model_dir = os.path.join(model_dir, f'resnet26x{model_width}', 'pairsplits')
    print(model_dir)
    ...
    if 'clip' in model_dir:
        ...
        out_dim = num_classes
    # lr 0001 cos0.5325
    # lr 0.1 e100 cos 0.7642
    # lr 0.1 e200 cos 0.7728
    # lr 0.1 e200 lambda 0.7756
    for _ in range(split_runs):
        splits = [np.arange(num_classes)]
        split_trainers = [
        ...
        for j in range(models_per_run):
            for i in range(len(splits)):
                model = resnet26(w=model_width, num_classes=out_dim).cuda().train()
                ...
                save_dir = os.path.join(model_dir, "0")
                os.makedirs(save_dir, exist_ok=True)
                save_path = os.path.join(save_dir, f'resnet26x{model_width}_v{len(os.listdir(save_dir))}.pth.tar')
