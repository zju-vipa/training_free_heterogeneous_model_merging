...
def prepare_train_loaders(config):
    if 'no_transform' in config:
        train_transform = T.Compose([T.Resize(56), T.ToTensor(), normalize])
    else:
        train_transform = T.Compose([
            T.Resize(56),T.RandomHorizontalFlip(), T.RandomCrop(56, padding=4), T.ToTensor(), normalize
        ])
    ...
    #                          generator=generator)[:config["train_sample"]]
    loaders = {
        'full':
...
def prepare_test_loaders(config):
    test_transform = T.Compose([T.Resize(56), T.ToTensor(), normalize])
