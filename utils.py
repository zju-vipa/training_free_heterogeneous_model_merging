...
def train_cliphead_adam(model,
                   train_loader,
                   test_loader,
                   class_vectors,
                   remap_class_idxs=None,
                   epochs=250,
                   lr=0.1,save_path=None,num_classes=10):
    ...
    """
    print("Ignore paramater", [name for name,p in model.named_parameters() if not p.requires_grad])
    optimizer = Adam([p for p in model.parameters() if p.requires_grad], lr=lr, weight_decay=5e-5)
    ...
    scheduler = lr_scheduler.CosineAnnealingLR(optimizer, epochs * ne_iters)
    scheduler = GradualWarmupScheduler(optimizer, multiplier=1., total_epoch=5 * ne_iters, after_scheduler=scheduler)
    ...
    for _ in pbar:
        ...
        pbar.set_description(f'prev acc: {acc}  lr: {scheduler.get_last_lr()} ')
...
def train_cliphead(model,
                   ...
                   remap_class_idxs=None,
                   epochs=200,
                   lr=0.4,save_path=None,warm_epoch=5,sche_type = ""):
    """Train a cliphead model.
    ...
    ne_iters = len(train_loader)
    lr_schedule = np.interp(np.arange(1 + epochs * ne_iters),
                            [0, warm_epoch * ne_iters, epochs * ne_iters], [0, 1, 0])
    
    if sche_type == "step":
        scheduler = lr_scheduler.StepLR(optimizer, step_size=30*ne_iters, gamma=0.1)
    else:
        scheduler = lr_scheduler.LambdaLR(optimizer, lr_schedule.__getitem__)
...
def train_model2(eval_type, 
    ...
    train_loader = config['data']['train']['splits'][0]
    ...
    if eval_type == 'logits':
        ...
        acc_overall, acc_avg, perclass_acc = train_cliphead_alltasks(
            model,
            train_loader,
            loader,
            class_vectors,
...
def train_cliphead_alltasks(model,train_loader, test_loader, label_encodings_list, splits,num_classes,epochs=5,lr=1e-5,
                               ):
    """Evaluate a cliphead on all tasks. Return acc_overall, acc_avg, perclass_acc
    ...
    model.train()
    optimizer = SGD([p for p in model.parameters() if p.requires_grad_], lr=lr, momentum=0.9, weight_decay=5e-4)
    ne_iters = len(train_loader)
    # lr_schedule = np.interp(np.arange(1 + epochs * ne_iters),
    #                         [0, 5 * ne_iters, epochs * ne_iters], [0, 1, 0])
    scheduler = lr_scheduler.CosineAnnealingLR(optimizer, epochs * ne_iters)
    ...
    label_remapping[all_splits.detach().cpu().numpy()] = np.arange(len(all_splits))
    label_remapping = torch.from_numpy(label_remapping).to(device)
    print("label remapping",label_remapping)
    label_encodings = torch.stack(label_encodings_list,
                                  dim=0).to(device)  # [S,C,D]
    multihead = False
    losses = []
    ...
    for epoch in pbar:
        model = model.train()
        pbar_inner = tqdm(train_loader,total=len(train_loader),desc="Training")
        for inputs, labels in pbar_inner:
            optimizer.zero_grad(set_to_none=True)
            with autocast():
                batch_size = inputs.shape[0]
                if batch_size == 0:
                ...
                encodings = encodings / encodings.norm(dim=-1, keepdim=True)
                if len(encodings.shape) == 3:
                    all_logits = 100 * torch.bmm(encodings,
                                        label_encodings.transpose(
                                            -1, -2)).transpose(1, 0)
                else:
                    all_logits = 100 * torch.einsum('be,sec->bsc', encodings,
                                            label_encodings.transpose(-1, -2))
                    
                all_logits = all_logits.flatten(1)
                remapped_labels = label_remapping[labels]
                loss = loss_fn(all_logits, remapped_labels)
            scaler.scale(loss).backward()
            scaler.step(optimizer)
            scaler.update()
            scheduler.step()
            losses.append(loss.item())
        acc, per_task,task_accs = evaluate_cliphead_alltasks(model,test_loader,label_encodings_list,splits,num_classes)
        pbar.set_description(f'prev acc: {acc} lr: {scheduler.get_last_lr()}')
        print(f'Epoch {epoch}, Acc: {acc}, Per {per_task}({task_accs})')
                # if multihead:
                #     all_logits = F.softmax(all_logits * 100, dim=-1)
 

    acc, per_task,task_accs = evaluate_cliphead_alltasks(model,test_loader,label_encodings_list,splits,num_classes)
    return acc, per_task,task_accs
...
def train_logits(model,
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,ne_iters * epochs)
    ...
    for epoch in pbar:
        ...
        print(
            f'Epoch: {epoch}. Best {best_acc}({best_epoch}), Acc {acc}, lr{scheduler.get_last_lr()}'
        )
    ...
    acc = evaluate_logits(model,
                          test_loader,
                          remap_class_idxs=remap_class_idxs)
    print('Acc at Best Model: {}'.format(best_acc))
...
def prepare_data(config, device='cuda'):
    ...
    elif data_config['type'] == 'cifar56':
        from datasets.cifar56 import prepare_train_loaders, prepare_test_loaders
        train_loaders = prepare_train_loaders(data_config)
        test_loaders = prepare_test_loaders(data_config)
...
def prepare_resnets(config, device):
    ...
    if 'x' in name:
        ...
        width = 1
    if "resnet50im" in name:
        from models.resnets import resnet50im as wrapper_w
        wrapper = lambda num_classes: wrapper_w(width, num_classes)
    elif "resnet34im" in name:
        from models.resnets import resnet34im as wrapper_w
        wrapper = lambda num_classes: wrapper_w(width, num_classes)
    elif "resnet50dino" in name:
        ...
        from models.resnets import resnet26 as wrapper_w
        wrapper = lambda num_classes: wrapper_w(width, num_classes)
    elif "resnet32" in name:
        from models.resnets import resnet32 as wrapper_w
        wrapper = lambda num_classes: wrapper_w(width, num_classes)
    elif 'resnet20' in name:
        from models.resnets import resnet20 as wrapper_w
        wrapper = lambda num_classes: wrapper_w(width, num_classes)
    elif 'resnet50' in name:
        from models.resnets import resnet50 as wrapper_w
        wrapper = lambda num_classes: wrapper_w(width, num_classes)
    elif 'resnet44' in name:
        from models.resnets import resnet44 as wrapper_w
        wrapper = lambda num_classes: wrapper_w(width, num_classes)
    elif 'resnet38' in name:
        from models.resnets import resnet38 as wrapper_w
        wrapper = lambda num_classes: wrapper_w(width, num_classes)
    elif 'resnet50tv' in name:
...
def prepare_vits(config, device):
    ...
    name = config['name']
    if "vith" in name:
        from models.vits import vithd as wrapper_w
        h_d = name[len("vith"):].split("d")
        wrapper = lambda num_classes: wrapper_w(num_classes=num_classes,num_heads=int(h_d[0]),depth=int(h_d[1]),pretrained=False)
    elif "vit1h7" in name:
        from models.vits import vit1h7 as wrapper_w
        wrapper = lambda num_classes: wrapper_w(num_classes)
    elif "vit1h19" in name:
        from models.vits import vit1h19 as wrapper_w
        wrapper = lambda num_classes: wrapper_w(num_classes)
    elif "vit7" in name:
        from models.vits import vit7 as wrapper_w
        wrapper = lambda num_classes: wrapper_w(num_classes)
    elif "vit19" in name:
        from models.vits import vit19 as wrapper_w
        wrapper = lambda num_classes: wrapper_w(num_classes)
    elif "vits" in name:
...
def prepare_vgg(config, device):
    ...
    elif 'vgg19' in config['name']:
        from models.vgg import vgg19 as wrapper_w
    elif 'vgg13' in config['name']:
        from models.vgg import vgg13 as wrapper_w
    else:
...
def inject_pair(config, pair, ignore_bases=False):
    ...
    if not ignore_bases:
        config['model']['bases'] = [
            os.path.join(config['model']['dir'], f"{split}",
