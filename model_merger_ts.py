...
from graphs_ts.base_graph import NodeType
from metric_calculators_ts import CovarianceMetric, MeanMetric
from matching_functions_ts import match_tensors_permute
from matching_functions_ts import compute_correlation
from utils_ts import getmembers, contains_name,get_merging_fn
...
class ModelMerge(nn.Module):
    ...
    def compute_transformations(self, transform_fn, reduce_ratio=.5, permute_heads=False, 
        ...
        # special_cases_names = ['pre_ln', 'attn_ln', 'emb_ln', 'q', 'k']
        special_cases_names = ['pre_ln', 'attn_ln', "emb", 'q', 'k']
        special_cases_nodes = [self.graphs[0].modules[name] for name in special_cases_names]
        qk_nodes = [self.graphs[0].modules[name] for name in ['q', 'k']]
    ...
    def apply_transformations_custom(self, merge_cls=False):
        def get_enc_prefix(enc_prefix):
            if enc_prefix != "":
                enc_prefix = f"{enc_prefix}."
            return enc_prefix
        qk_flag = False
        if self.graphs[0].qk == True:
            qk_flag = True
        ...
        emb_copy_0 = self.graphs[0].get_module(f'{get_enc_prefix(self.graphs[0].enc_prefix)}{emb_suff_0}').weight.data
        emb_copy_0 = torch.clone(emb_copy_0)

        emb_suff_1 = self.graphs[1].modules['emb']
        emb_copy_1= self.graphs[1].get_module(f'{get_enc_prefix(self.graphs[1].enc_prefix)}{emb_suff_1}').weight.data
                ...
                if info['type'] == NodeType.SUM:
                    ...
                    if info['type'] == NodeType.SUM:
                        ...
                        if merger.graph.enc_prefix == 'bert':
                            ...
                            emb_tok_name = f'{get_enc_prefix(merger.graph.enc_prefix)}{emb_tok_suff}'
                        ...
                        emb_name = f'{get_enc_prefix(merger.graph.enc_prefix)}{emb_suff}'
                        emb_pos_name = f'{get_enc_prefix(merger.graph.enc_prefix)}{emb_pos_suff}'
