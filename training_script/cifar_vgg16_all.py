...
from models.vgg import vgg16,vgg11,vgg13
import torch
import torchvision
...
if __name__ == "__main__":
    ...
    use_clip = True
    model_dir = f'./checkpoints_all/cifar5_{"clip" if use_clip else "logits"}'
    ...
    model_width = 2                        # resnet model width
    epochs = 200                            # train epochs
    
    model_dir = os.path.join(model_dir, f'vgg16x{model_width}', 'pairsplits')
    print(model_dir)
    ...
    for _ in range(split_runs):
        ...
        for j in range(models_per_run):
            for i in range(len(splits)):
                model = vgg16(w=model_width, num_classes=out_dim).cuda().train()
                ...
                save_path = os.path.join(save_dir, f'vgg16x{model_width}_v{len(os.listdir(save_dir))}.pth.tar')
