...
class ResNetIM(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = nn.Conv2d(3, int(w * 16), kernel_size=7, stride=2, padding=3, bias=False)
        self.relu1 = nn.ReLU(inplace=True)
        self.norm_layer = norm_layer
        self.bn1 = norm_layer(int(w * 16))
        ...
        if text_head:
            num_classes = 512
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.linear = nn.Linear(int(w * 128), num_classes)
        self.flatten = nn.Flatten()
...
def resnet26(w=1, num_classes=10, text_head=False):
    return ResNet(BasicBlock, [4, 4, 4],
                  w=w,
                  num_classes=num_classes,
                  text_head=text_head)

def resnet32(w=1, num_classes=10, text_head=False):
    return ResNet(BasicBlock, [5, 5, 5],
                  w=w,
                  num_classes=num_classes,
                  text_head=text_head)

def resnet38(w=1, num_classes=10, text_head=False):
    return ResNet(BasicBlock, [6, 6, 6],
                  w=w,
                  num_classes=num_classes,
                  text_head=text_head)

def resnet44(w=1, num_classes=10, text_head=False):
    return ResNet(BasicBlock, [7, 7, 7],
                  w=w,
                  num_classes=num_classes,
                  text_head=text_head)

def resnet50(w=1, num_classes=10, text_head=False):
    return ResNet(BasicBlock, [8, 8, 8],
                  w=w,
                  num_classes=num_classes,
                  text_head=text_head)

def resnet50im(w=1, num_classes=1000, text_head=False):
    return ResNetIM(BasicBlock, [6, 6, 6,6],
                  w=w,
                  num_classes=num_classes,
                  text_head=text_head)

def resnet34im(w=1, num_classes=1000, text_head=False):
    return ResNetIM(BasicBlock, [4, 4, 4, 4],
                  w=w,
                  num_classes=num_classes,
                  text_head=text_head)


def resnet50dil(num_classes=1000, text_head=False):
    return torchvision_models.resnet50(num_classes=num_classes,replace_stride_with_dilation=[False, True, True])

def resnet101dil(num_classes=1000, text_head=False):
    return torchvision_models.resnet101(num_classes=num_classes,replace_stride_with_dilation=[False, True, True])
