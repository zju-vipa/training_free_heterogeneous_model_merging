...
from utils import *
from models.resnets import resnet101dil
import torch
import torchvision
...
if __name__ == "__main__":
    ...
    # task_split_dict = {k:list(range(1000)) for k in range(1)}
    task_split_dict = {k:list(range(500*k,500*(k+1))) for k in range(2)}

    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    use_clip = True
    model_dir = f'./checkpoints/imnet1k_{"clip" if use_clip else "logits"}'
    ...
    model_dir = os.path.join(model_dir, f'resnet101dil', 'pairsplits')
    print(model_dir)
    ...
    save_path = os.path.join(
            save_dir,
            f'resnet101dil_v{len(os.listdir(save_dir))}.pth.tar')
    for j in range(models_per_run):
        model = resnet101dil(num_classes=out_dim).cuda().train()
