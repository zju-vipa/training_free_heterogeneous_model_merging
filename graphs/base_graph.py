...
class BIGGraph(ABC):
    ...
    def _check_dummy_module(self, m: nn.Module):
        # 如果是空模块就不激活了
        if isinstance(m,(nn.Conv2d,nn.Linear,nn.LayerNorm)):
            if all(torch.all(p==0).cpu().detach().item() for p in m.parameters()):
                # print("Skip dummy module", type(m))
                return True
        elif isinstance(m, nn.BatchNorm2d):
            if all(torch.all(p==0).cpu().detach().item() for p in [m.weight,m.bias,m.running_mean]):
                return True
        return False
            ...
            if info['type'] == NodeType.PREFIX:
                ...
                for succ_node in self.G.succ[node]:
                    ...
                    if succ_info['type'] == NodeType.MODULE:
                        ...
                        if not self._check_dummy_module(module):
