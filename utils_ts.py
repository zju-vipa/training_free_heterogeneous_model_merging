...
from metric_calculators_ts import get_metric_fns
import random
...
def prepare_graph(config, classifier=False):
    ...
    if config['name'].startswith('bert'):
        import graphs_ts.transformer_enc_graph as graph_module
        model_name = 'bert'
...
def get_merging_fn(name):
    ...
    import matching_functions_ts
    matching_fns = dict([(k, v) for (k, v) in getmembers(matching_functions_ts, isfunction) if 'match_tensors' in k])
