...
if __name__ == '__main__':
    ...
    model = resnet2.resnet50im().eval()
    state_dict = model.state_dict()

    print(model)


    graph1 = ResNetGraph(deepcopy(model),num_layers=4).graphify()
    # graph2 = resnet20(deepcopy(model)).graphify()

    graph1.draw(save_path="graph")

    # merge = ModelMerge(graph1, graph2)
    # merge.transform(model3, dataloader, transform_fn=match_tensors_zipit)

    # graph1.draw(nodes=range(20),save_path="graph.png")
    # graph1.draw(nodes=range(len(graph1.G)-20, len(graph1.G)),save_path="graph.png")

    # print(model.eval().cuda()(data_x.cuda()))

    # print(merge(data_x.cuda()))

    # print("Nodenum",len(resnet20(deepcopy(model)).graphify().G.nodes))
