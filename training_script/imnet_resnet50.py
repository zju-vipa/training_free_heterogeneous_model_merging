...
from utils import *
from models.resnets import resnet50im
import torch
import torchvision
...
if __name__ == "__main__":
    ...
    model_dir = os.path.join(model_dir, f'resnet50imx{model_width}', 'pairsplits')
    print(model_dir)
    ...
    save_path = os.path.join(
            save_dir,
            f'resnet50imx{model_width}_v{len(os.listdir(save_dir))}.pth.tar')
    for j in range(models_per_run):
        model = resnet50im(num_classes=out_dim,w=model_width).cuda().train()
